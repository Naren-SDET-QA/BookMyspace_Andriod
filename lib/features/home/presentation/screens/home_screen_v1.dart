// release/v1.0 version of home_screen.dart, kept alongside the
// main-lineage screen so none of its behaviour is lost. Public classes that
// clash with the main version carry a V1 suffix.
...
enum MainHomeSectionV1 {
  ...
  String get id {
    switch (this) {
      case MainHomeSectionV1.functionHalls:
        return 'function_halls';
      case MainHomeSectionV1.lodgeRooms:
        return 'lodge_rooms';
      case MainHomeSectionV1.pgHostels:
        return 'pg_hostels';
      case MainHomeSectionV1.institutesClasses:
...
class HomeScreenV1 extends ConsumerStatefulWidget {
  const HomeScreenV1({super.key, this.initialSection, this.forceModernLayout});
...
class _HomeScreenState extends ConsumerState<HomeScreenV1> {
  final Set<String> _selectedAmenities = {};

  List<MainHomeSectionV1> _visibleHomeSections(WidgetRef ref) {
    ...
    final effective = HomeCategoryCatalog.effectiveMainSections(
      ...
      customerEnabledIds: MainHomeSectionV1.values
          .where((section) => preferences.isEnabled(section.id))
          .map((section) => section.id)
          .toSet(),
    );
    return MainHomeSectionV1.values
  ...
  List<HomeCategoryItem> _visibleConfiguredHomeCategories(WidgetRef ref) {
    ...
    final mainSectionIds = MainHomeSectionV1.values.map((s) => s.id).toSet();
  ...
  Widget build(BuildContext context) {
    ...
    final selectedSection = selectedCatalog == null
        ? null
        : MainHomeSectionV1.values.firstWhere(
            (s) => s.id == selectedCatalog.id,
            orElse: () => MainHomeSectionV1.functionHalls,
          );
                  ...
                  // =========================================================
                            ...
                            if (selectedSection.catalog.isBookable) ...[
                              ...
                              _QuickBookCard(
                                ...
                                onQuickBookTap: () {
                                  ...
                                  context.push(
                                    AppRoutes.v1BookingFlow.replaceAll(
                                      ':id',
                                      match.id,
                                    ),
                                    extra: match,
                    ...
                    popularVenuesAsync.when(
                      data: (venues) {
                        ...
                        if (selectedSection == MainHomeSectionV1.lodgeRooms) {
  ...
  List<CategoryConfiguration> _carouselCategories(
    MainHomeSectionV1 selectedSection,
              ...
              FilledButton(
                onPressed: () {
                  ...
                  if (action == HelpAction.booking && section != null) {
                    ...
                    if (match != null) {
                      context.push(
                        AppRoutes.v1BookingFlow.replaceAll(':id', match.id),
                        extra: match,
                      );
                    } else {
                      context.push(
