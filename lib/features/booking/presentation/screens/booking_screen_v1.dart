// release/v1.0 version of booking_screen.dart, kept alongside the
...
class BookingScreenV1 extends ConsumerStatefulWidget {
  const BookingScreenV1({super.key, required this.venue});

  final Venue venue;

  @override
  ConsumerState<BookingScreenV1> createState() => _BookingScreenState();
}

class _BookingScreenState extends ConsumerState<BookingScreenV1> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(CustomerSectionCatalog.bookingScreenTitle(section)),
        actions: [
          const ContextualHelpButton(route: AppRoutes.v1BookingFlow),
  ...
  Future<void> _confirmBooking(DateTime date) async {
    ...
    try {
      ...
      if (isCheckoutExposed(ref.read(featureRegistryProvider))) {
        unawaited(context.push(AppRoutes.v1PaymentFlow.replaceAll(':id', booking.id), extra: booking));
      }
    } catch (e) {
      if (!mounted) return;
